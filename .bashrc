...
function kepignore-crashes { target="$(hack-home "ignore-crashes")"; if [[ $? == 0 ]]; then cd "$target"; fi }
function sbignore-crashes { target="$(hack-home "ignore-crashes")/sandbox"; if [[ $? == 0 ]]; then cd "$target"; fi }
